...
test("the kubectl command is the one you could actually paste", () => {
  ...
  // The group qualifies the type, before the name.
  assert.equal(
    kubectlFor({ subject: { kind: "User", name: "ada" }, verb: "get", resource: "deployments", apiGroup: "apps", resourceName: "web", namespace: "dev" }),
    "kubectl auth can-i get deployments.apps/web -n dev --as=ada",
  );
  // A namespaced resource with no namespace is a question about all of them.
  assert.equal(
    kubectlFor({ subject: { kind: "User", name: "ada" }, verb: "list", resource: "pods" }),
    "kubectl auth can-i list pods --all-namespaces --as=ada",
  );
});

test("a RoleBinding never grants a namespaced resource across all namespaces", () => {
  const cluster = {
    roles: [{ kind: "ClusterRole", name: "pod-reader", rules: [{ apiGroups: [""], resources: ["pods"], verbs: ["list"] }] }],
    bindings: [
      { kind: "RoleBinding", name: "rb", namespace: "dev", roleRef: { kind: "ClusterRole", name: "pod-reader" }, subjects: [sa("ci", "dev")] },
    ],
  };
  const all = can(cluster, { subject: sa("ci", "dev"), verb: "list", resource: "pods" });
  assert.equal(all.allowed, false);
  assert.match(all.nearMisses[0].reason, /ClusterRoleBinding/);
  assert.match(all.summary, /across all namespaces/);
  assert.equal(can(cluster, { subject: sa("ci", "dev"), verb: "list", resource: "pods", namespace: "dev" }).allowed, true);
  // A ClusterRoleBinding does grant it.
  cluster.bindings[0] = { kind: "ClusterRoleBinding", name: "crb", roleRef: { kind: "ClusterRole", name: "pod-reader" }, subjects: [sa("ci", "dev")] };
  assert.equal(can(cluster, { subject: sa("ci", "dev"), verb: "list", resource: "pods" }).allowed, true);
});
